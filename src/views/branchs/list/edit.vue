<template>
    <div class="wrap-main">
        ...
        <a-card class="general-card" :title="'Sửa thông tin chi nhánh'">
            <a-row class="wrapper">
                <a-col :span="24">
                    <a-form ref="formRef" label-align="left" :label-col-props="{ span: 10, offset: 0 }" :wrapper-col-props="{ span: 24 }" :model="dataForm" class="form">
                        ...
                        <a-row :gutter="24" :justify="'center'">
                            <a-col style="margin-top: 20px">
                                <a-form-item>
                                    <a-space>
                                        <a-button type="primary" @click="validate({ errors: formRef.value?.getFieldsError(), values: dataForm })"> Sửa thông tin </a-button>
...
<script lang="ts" setup>
    ...
    const breadcrumbRoutes = [
        ...
        { path: `/branchs/edit/${route.params.id}`, label: 'Sửa thông tin chi nhánh' },
    ];
    ...
    try {
        ...
        if (branch) {
            dataForm.value = {
                ...
                logo: branch.logo,
                openTime: branch.openTime ? branch.openTime.split("T")[1].slice(0,5) : '',
                closeTime: branch.closeTime ? branch.closeTime.split("T")[1].slice(0,5) : '',
            }
            
            if(branch.thumbnail){
              defaultFileList.value.push({ id: '1', name: 'thumbnail', url: branch.thumbnail });
            }
            if(branch.logo){
              defaultFileList.value.push({ id: '2', name: 'logo', url: branch.logo });
            }
        }
    } catch (err) {
        console.error(err)
    ...
    const validate = async ({ errors, values }: { errors: Record<string, ValidatedError> | undefined; values: Record<string, any> }) => {
        ...
        if (!errors) {
            ...
            try {
              ...
              useBranchStore().updateBranch(route.params.id as string,dataForm.value);
    ...
    loadBranch(route.params.id as string);
</script>
